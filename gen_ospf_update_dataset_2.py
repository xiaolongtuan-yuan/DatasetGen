...
def ospf_net_gen(sub_directorie, net_id, node_account):
    df = pd.DataFrame(columns=column_names)

    # node_account = random.randint(*graph_size)
    builder = NetworkBuilder(node_account, f'ospf_update_dataset/{sub_directorie}/{net_id}')
    builder.build_graph()
    # builder.draw_graph()
    pre_weight_matrix = builder.weight_matrix
    with open(os.path.join(f'ospf_update_dataset/{sub_directorie}/{net_id}', "pre_weight_matrix.pkl"), 'wb') as file:
    ...
    with open(os.path.join(f'ospf_update_dataset/{sub_directorie}/{net_id}', "cur_weight_matrix.pkl"), 'wb') as file:
    ...
    ned_path = os.path.join(f'ospf_update_dataset/omnet_file/{sub_directorie}/networks', f"Myned{str(net_id)}.ned")
    ini_path = os.path.join(f'ospf_update_dataset/omnet_file/{sub_directorie}/ini_dir', f"omnetpp{str(net_id)}.ini")
    ...
    new_df.to_json(f"ospf_update_dataset/{sub_directorie}/{net_id}/dataset.jsonl", orient="records", lines=True)
    return 1
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    ...
    for data_size in data_sizes:
        for i in range(data_size['account']):
            ...
            futures[executor.submit(ospf_net_gen, sub_directorie, i, node_account)] = i
